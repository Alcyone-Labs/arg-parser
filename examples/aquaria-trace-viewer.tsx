...
const MOCK_DBS: DbFile[] = [
  { path: "/data/traces-2024-01.sqlite", name: "traces-2024-01.sqlite", size: 15_728_640, traceCount: 1247 },
  { path: "/data/production.sqlite", name: "production.sqlite", size: 104_857_600, traceCount: 15892 },
  { path: "/data/dev-local.sqlite", name: "dev-local.sqlite", size: 2_097_152, traceCount: 89 },
...
const MOCK_TRACES: Trace[] = [
  { id: "tr_001", workflowName: "rss-processor", status: "success", duration: 1200, tokens: 15000, cost: 0.045, stepCount: 5 },
  { id: "tr_002", workflowName: "embed-content", status: "error", duration: 500, tokens: 750, cost: 0.024, stepCount: 3 },
  { id: "tr_003", workflowName: "classify-item", status: "success", duration: 800, tokens: 12000, cost: 0.036, stepCount: 4 },
  ...Array.from({ length: 147 }, (_, i) => ({
    id: `tr_${100+i}`,
    workflowName: ["daily-summary", "email-sender", "data-sync", "pdf-extractor", "rag-indexer"][i % 5]!,
...
const MOCK_STEPS: Step[] = [
  { id: "s1", name: "fetch-content", type: "tool", status: "success", duration: 150, input: '{"url": "https://example.com/article/1"}' , output: '{"html": "..."}' },
  { id: "s2", name: "parse-html", type: "capability", status: "success", duration: 50, input: '{"html": "..."}', output: '{"text": "Content..."}' },
  { id: "s3", name: "embed-text", type: "llm", status: "success", duration: 300, input: '{"text": "..."}', output: '{"vector": [0.1, ...]}' },
  { id: "s4", name: "classify", type: "llm", status: "success", duration: 200, input: '{"text": "..."}', output: '{"category": "tech"}' },
  { id: "s5", name: "store-result", type: "tool", status: "success", duration: 50, input: '{"data": "..."}', output: '{"id": "stored_123"}' },
];
...
function useVirtualScroll<T>(items: () => T[], selectedIdx: () => number, viewportHeight: () => number) {
  ...
  return (
    <box width="100%" height="100%" flexDirection="column" backgroundColor={t().bg}>
      ...
      <box height={3} borderStyle="single" borderColor={t().accent} justifyContent="center" alignItems="center">
        <text bold color={t().accent}> 🔍 Aquaria Trace Viewer </text>
      </box>
      
      {/* Breadcrumb */}
      <box height={1} paddingLeft={2} backgroundColor={t().bg}>
         <text color={t().accent} bold>Databases</text>
         <Show when={level() >= 2}>
           <text color={t().muted}> › </text>
           <text color={t().accent} bold>{selectedDb().name}</text>
         </Show>
         <Show when={level() >= 3}>
           <text color={t().muted}> › </text>
           <text color={t().accent} bold>{selectedTrace().workflowName}</text>
         </Show>
      </box>
      ...
      <box flexGrow={1} flexDirection="row">
        ...
        <box width="35%" borderStyle="single" borderColor={t().border} flexDirection="column" padding={1}>
           ...
           <Show when={level() === 1}>
             <text bold color={t().fg} marginBottom={1}>Databases ({selectedDbIdx() + 1}/{MOCK_DBS.length})</text>
             <For each={dbScroll.visibleItems()}>
               {({ item, globalIndex }) => (
                 <box height={1} backgroundColor={globalIndex === selectedDbIdx() ? t().selection : undefined}>
                   <text color={globalIndex === selectedDbIdx() ? t().selectionFg : t().fg}>
                     {globalIndex === selectedDbIdx() ? "› " : "  "}{item.name}
                   </text>
                 </box>
               )}
             </For>
           ...
           <Show when={level() === 2}>
             <text bold color={t().fg} marginBottom={1}>Traces ({selectedTraceIdx() + 1}/{MOCK_TRACES.length})</text>
             <For each={traceScroll.visibleItems()}>
               {({ item, globalIndex }) => (
                 <box height={1} backgroundColor={globalIndex === selectedTraceIdx() ? t().selection : undefined}>
                   <text color={globalIndex === selectedTraceIdx() ? t().selectionFg : t().fg}>
                     {globalIndex === selectedTraceIdx() ? "› " : "  "}{item.workflowName}
                   </text>
                 </box>
               )}
             </For>
           ...
           <Show when={level() === 3}>
             <text bold color={t().fg} marginBottom={1}>Steps ({selectedStepIdx() + 1}/{MOCK_STEPS.length})</text>
             <For each={stepScroll.visibleItems()}>
               {({ item, globalIndex }) => (
                 <box height={1} backgroundColor={globalIndex === selectedStepIdx() ? t().selection : undefined}>
                   <text color={globalIndex === selectedStepIdx() ? t().selectionFg : t().fg}>
                     {globalIndex === selectedStepIdx() ? "› " : "  "}{item.name}
                   </text>
                 </box>
               )}
             </For>
        ...
        <box flexGrow={1} borderStyle="single" borderColor={t().border} flexDirection="column" padding={2}>
           ...
           <Show when={level() === 1}>
              <text bold color={t().accent}>{selectedDb().name}</text>
              <text color={t().muted} marginBottom={2}>{selectedDb().path}</text>
              <text color={t().fg}>Size: {formatBytes(selectedDb().size)}</text>
              <text color={t().fg}>Traces: {selectedDb().traceCount}</text>
           </Show>

           <Show when={level() === 2}>
              <text bold color={t().accent}>{selectedTrace().workflowName}</text>
              <text color={selectedTrace().status === "success" ? t().success : t().error} bold>{selectedTrace().status.toUpperCase()}</text>
              <text color={t().muted} marginBottom={1}>ID: {selectedTrace().id}</text>
              <box flexDirection="column" borderStyle="single" borderColor={t().muted} padding={1} marginTop={1}>
           ...
           <Show when={level() === 3}>
              <text bold color={t().accent}>{selectedStep().name} ({selectedStep().type})</text>
              <text color={t().muted} marginBottom={1}>Duration: {formatDuration(selectedStep().duration)}</text>
              <text bold color={t().fg} marginTop={1}>Input:</text>
              <text color={t().muted}>{selectedStep().input}</text>
              <text bold color={t().fg} marginTop={1}>Output:</text>
      ...
      <box height={1} backgroundColor={t().bg}>
         <text color={t().muted}> ←: Back | ↑↓/jk: Nav | →: Open | Scroll: Mouse | t: Theme | q: Quit</text>
      </box>
    </box>
  );
}
