...
async function loadArgParserFromFile(filePath: string): Promise<ArgParser> {
  ...
  try {
    ...
    throw new Error(
      `Failed to load ArgParser from ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function formatTextReport(report: FuzzyTestReport): string {
  ...
  lines.push(
    `  Successful: ${report.successfulTests} (${((report.successfulTests / report.totalTests) * 100).toFixed(1)}%)`,
  );
  lines.push(
