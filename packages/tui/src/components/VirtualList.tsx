...
export function VirtualList<T>(props: VirtualListProps<T>): JSX.Element {
  ...
  // Scroll by delta (exposed via VirtualListResult, not used internally)
  const _scrollBy = (delta: number) => {
    const maxOffset = Math.max(0, items().length - viewportHeight());
    setScrollOffset((o) => Math.max(0, Math.min(maxOffset, o + delta)));
  };

  // Navigation helpers (available for external consumers)
  ...
  // Suppress unused-local warnings for navigation helpers reserved for future use
  void _scrollBy;
  ...
  const defaultRenderItem = (item: T, _index: number, selected: boolean) => {
