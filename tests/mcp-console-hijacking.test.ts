  ...
  describe("CLI Mode Console Behavior", () => {
    test("should output to stdout in CLI mode", async () => {
      ...
      .addTool({
        ...
        flags: [
          { name: "message", description: "Message to output", options: ["--message"], type: "string", mandatory: true }
        ],
        handler: async (ctx) => {
  ...
  describe("MCP Mode Console Hijacking", () => {
    ...
    test("should prevent stdout contamination during tool execution", async () => {
      ...
      .addTool({
        ...
        flags: [
          { name: "count", description: "Number of messages", options: ["--count"], type: "number", defaultValue: 3 }
        ],
        handler: async (ctx) => {
          ...
          for (let i = 0; i < ctx.args.count; i++) {
            ...
            console.log(`   Status: active | Progress: ${((i + 1) / ctx.args.count * 100).toFixed(0)}%`);
          }
