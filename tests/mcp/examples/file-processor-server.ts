...
const cli = ArgParser.withMcp({
  ...
  description: "Advanced file processing tools for AI assistants. Provides file analysis, transformation, and management capabilities.",
  handler: async (ctx) => {
...
.addSubCommand({
  ...
  handler: async (ctx) => {
    ...
    const result: any = {
      ...
      preview: transformedContent.substring(0, 200) + (transformedContent.length > 200 ? "..." : "")
    };
...
.addSubCommand({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      recursive,
      caseSensitive,
      totalFiles: stats.isDirectory() ? (recursive ? getFilesRecursively(searchPath).length : getFilesInDirectory(searchPath).length) : 1,
...
.addMcpSubCommand("serve", {
  ...
  version: "1.0.0",
  description: "File processing MCP server providing advanced file analysis, transformation, and search capabilities for AI assistants"
}, {
  defaultTransports: [
    { type: "stdio" },
...
function searchInFile(filePath: string, pattern: string, caseSensitive: boolean): Array<{lineNumber: number, line: string, match: string}> {
  const content = readFileSync(filePath, "utf-8");
  const lines = content.split("\n");
