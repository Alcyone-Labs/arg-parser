...
`;
      ...
      const argParserImportIndex = lines.findIndex((line) =>
        line.includes("ArgParser"),
      );
      const chalkImportIndex = lines.findIndex((line) =>
        line.includes("chalk"),
      );
      const loggerImportIndex = lines.findIndex((line) =>
        line.includes("createMcpLogger"),
      );
      ...
      expect(loggerImportIndex).toBeGreaterThan(
        Math.max(argParserImportIndex, chalkImportIndex),
      );
    });
...
`;
      ...
      const result = (dxtGenerator as any)["processCliSourceForMcp"](
        realisticCliSource,
      );
