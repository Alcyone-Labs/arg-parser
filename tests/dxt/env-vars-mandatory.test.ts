...
describe('DXT Environment Variables Mandatory Behavior', () => {
  ...
  test('should mark all environment variables as required in user_config regardless of flag mandatory status', () => {
    ...
    const { envVars, userConfig } = (dxtGenerator as any).generateEnvAndUserConfig();
    ...
    expect(envVars).toHaveProperty('API_ENDPOINT', '${user_config.API_ENDPOINT}');
    ...
    expect(userConfig.API_KEY).toHaveProperty('description', 'API key for service');
    expect(userConfig.AUTH_TOKEN).toHaveProperty('description', 'Authentication token');
    expect(userConfig.API_ENDPOINT).toHaveProperty('description', 'API endpoint URL');
    expect(userConfig.SECRET_KEY).toHaveProperty('description', 'Secret key for processing');
  });
...
parser.parse(process.argv.slice(2));
    ...
    const manifestPath = path.join(tempDir, 'env-test-server-dxt', 'manifest.json');
    ...
    expect(manifest.user_config.API_KEY).toHaveProperty('description', 'API key');

    // Verify server env mapping
    expect(manifest.server.mcp_config).toHaveProperty('env');
    expect(manifest.server.mcp_config.env).toHaveProperty('API_KEY', '${user_config.API_KEY}');
  });
});
