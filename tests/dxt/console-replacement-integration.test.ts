...
`;
    ...
    const processedContent = (dxtGenerator as any)['processCliSourceForMcp'](testCliContent);

    // Verify global console replacement setup
    expect(processedContent).toContain('import { createMcpLogger } from \'@alcyone-labs/arg-parser\';');
    expect(processedContent).toContain('const mcpLogger = createMcpLogger(\'[CLI]\');');
    expect(processedContent).toContain('globalThis.console = {');
    expect(processedContent).toContain('log: (...args) => mcpLogger.info(...args)');
    expect(processedContent).toContain('warn: (...args) => mcpLogger.warn(...args)');
    expect(processedContent).toContain('info: (...args) => mcpLogger.info(...args)');
    expect(processedContent).toContain('debug: (...args) => mcpLogger.debug(...args)');
    ...
    expect(processedContent).toContain('console.log(chalk.green(\'Starting test...\'));');
    ...
    expect(processedContent).toContain('console.error(\'Error message should remain\');');
  });
...
`;
    ...
    expect(processedContent.trim().startsWith('import { createMcpLogger }')).toBe(true);
    ...
    expect(processedContent).toContain('console.warn(\'Warning: deprecated API\');');
