...
describe("Inheritance Mechanics", () => {
  test("Case A: Bottom-Up Construction (Legacy/DirectParentOnly) - Should NOT inherit grandparent flags recursively upon attachment", () => {
    ...
    rootParser.addFlag({ name: "root-flag", type: String, options: ["--root"] });
  
    // 2. Middle Parser (inheritParentFlags: true => DirectParentOnly (Snapshot))
    const midParser = new ArgParser({ appName: "mid", inheritParentFlags: true });
    midParser.addFlag({ name: "mid-flag", type: String, options: ["--mid"] });
  
    // 3. Leaf Parser (inheritParentFlags: true => DirectParentOnly (Snapshot))
    const leafParser = new ArgParser({ appName: "leaf", inheritParentFlags: true });
    leafParser.addFlag({ name: "leaf-flag", type: String, options: ["--leaf"] });
  ...
  test("Case C: Bottom-Up Construction (FlagInheritance.AllParents) - Should inherit grandparent flags via propagation", () => {
    ...
    const midParser = new ArgParser({ appName: "midC", inheritParentFlags: FlagInheritance.AllParents });
    midParser.addFlag({ name: "mid-flag", type: String, options: ["--mid"] });
  
    const leafParser = new ArgParser({ appName: "leafC", inheritParentFlags: FlagInheritance.AllParents });
    leafParser.addFlag({ name: "leaf-flag", type: String, options: ["--leaf"] });
  ...
  test("FlagInheritance.DirectParentOnly should match legacy boolean true", () => {
    ...
    const midParser = new ArgParser({ appName: "mid", inheritParentFlags: FlagInheritance.DirectParentOnly });
    midParser.addFlag({ name: "mid-flag", type: String, options: ["--mid"] });
  
    const leafParser = new ArgParser({ appName: "leaf", inheritParentFlags: FlagInheritance.DirectParentOnly });
    leafParser.addFlag({ name: "leaf-flag", type: String, options: ["--leaf"] });
